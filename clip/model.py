...
class ModifiedResNet_FEDOTP(nn.Module):
    """
    A ResNet class that is similar to torchvision's but contains the following changes:
    - There are now 3 "stem" convolutions as opposed to 1, with an average pool instead of a max pool.
    - Performs anti-aliasing strided convolutions, where an avgpool is prepended to convolutions with stride > 1
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            # Corresponds to default CoOp-style trainers (and similar prompt-based variants)
            assert current_trainer in [
                "promptfl",
                "fedotp",
                "fedpgp",
                "fedpha",
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_tokens: bool = False):
        # Align input dtype/device with conv1 weights to avoid half/float mismatch
        ...
        # Store tokens before pooling for return_tokens mode
        tokens = x  # [B, L, width] where L includes cls token and patches
        ...
        if return_tokens:
            # Return both pooled feature and tokens
            # tokens: [B, L, width] - includes cls token and all patch tokens
            return x, tokens
        
        return x


class VisionTransformer_FEDOTP(nn.Module):
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)): # ResNet 架构
            vision_heads = vision_width * 32 // 64
            if trainer == "FEDOTP":
                self.visual = ModifiedResNet_FEDOTP(
            ...
            if trainer == "FEDOTP":
                self.visual = VisionTransformer_FEDOTP(
                    input_resolution=image_resolution,
