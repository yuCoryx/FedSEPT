...
class DescribableTextures(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        if num_shots >= 1:
            ...
            if os.path.exists(preprocessed):
                logging.info(f"Loading preprocessed few-shot data from {preprocessed}")
                ...
                logging.info(f"Saving preprocessed few-shot data to {preprocessed}")
        ...
        if cfg.DATASET.USERS > 0 and cfg.DATASET.USEALL:
            ...
            logging.info("federated all dataset")
            ...
            federated_test_x = self.generate_federated_dataset(test, num_shots=num_shots,
                                                                num_users=cfg.DATASET.USERS, is_iid=cfg.DATASET.IID,
                                                                repeat_rate=cfg.DATASET.REPEATRATE)
            logging.info("fewshot federated dataset")
        else:
    ...
    def read_and_split_data(image_dir, p_trn=0.5, p_val=0.2, ignored=[], new_cnames=None):
        ...
        p_tst = 1 - p_trn - p_val
        logging.info(f"Splitting into {p_trn:.0%} train, {p_val:.0%} val, and {p_tst:.0%} test")
