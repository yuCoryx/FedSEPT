...
def read_image(path):
    ...
    try:
        ...
        # 如果文件不存在，创建一个占位符图像（黑色图像）
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"Image file not found: {path}, using placeholder image")
        # 创建一个 224x224 的黑色 RGB 图像作为占位符
        return Image.new("RGB", (224, 224), color=(0, 0, 0))
    except Exception as e:
        # 处理其他可能的错误（如损坏的图像文件）
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"Error reading image {path}: {e}, using placeholder image")
        return Image.new("RGB", (224, 224), color=(0, 0, 0))
