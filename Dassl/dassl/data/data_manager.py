...
def build_data_loader(
    ...
    if data_source is None or len(data_source) == 0:
        logging.warning(f"Empty data_source provided to build_data_loader. Returning None.")
        return None
    ...
    # Only assert if data_source is not empty (we already checked above)
    if len(data_source) > 0:
        assert len(data_loader) > 0, f"DataLoader is empty but data_source has {len(data_source)} samples (batch_size={batch_size}, drop_last={is_train and len(data_source) >= batch_size})"
    ...
    def __init__(
        ...
        if dataset.federated_train_x:
            for idx in range(cfg.DATASET.USERS):
                if len(dataset.federated_train_x[idx]) == 0:
                    logging.warning(f"Client {idx} has empty training data. Skipping data loader creation.")
                    continue
                ...
                if fed_train_loader_x is not None:
        ...
        if dataset.federated_test_x:
            for idx in range(cfg.DATASET.USERS):
                if len(dataset.federated_test_x[idx]) == 0:
                    logging.warning(f"Client {idx} has empty test data. Skipping data loader creation.")
                    continue
                ...
                if fed_test_loader_x is not None:
                    fed_test_loader_x_dict[idx] = fed_test_loader_x
 
 
        fed_test_neighbor_loader_x_dict = defaultdict()
        if dataset.federated_test_x:
            # 获取邻居测试集的batch size（如果配置了，否则使用默认测试batch size）
            neighbor_batch_size = getattr(cfg.DATALOADER.TEST, 'NEIGHBOR_BATCH_SIZE', cfg.DATALOADER.TEST.BATCH_SIZE)
            if neighbor_batch_size > cfg.DATALOADER.TEST.BATCH_SIZE:
                logging.info(f"Using larger batch size {neighbor_batch_size} for neighbor test set (default: {cfg.DATALOADER.TEST.BATCH_SIZE})")
            ...
            for idx in range(cfg.DATASET.USERS):
                ...
                if len(data_source) == 0:
                    logging.warning(f"Client {idx} has empty neighbor test data. Skipping data loader creation.")
                    continue
                fed_test_neighbor_loader_x = build_data_loader(
                    cfg,
                    sampler_type=cfg.DATALOADER.TEST.SAMPLER,
                    data_source=data_source,
                    batch_size=neighbor_batch_size,  # 使用更大的batch size
                    tfm=tfm_test,
                    is_train=False,
                    dataset_wrapper=dataset_wrapper
                )
                if fed_test_neighbor_loader_x is not None:
                    fed_test_neighbor_loader_x_dict[idx] = fed_test_neighbor_loader_x


        # Build test_loader 全通用/全局测试集
